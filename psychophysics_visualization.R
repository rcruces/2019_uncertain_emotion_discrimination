...
heatmap.2(calAccu(hap)[indx,],Colv=FALSE,main= "Fine Structure Accuracy HAP",
          density.info="histogram",Rowv = FALSE,
          dendrogram='none',
          trace="row",
          tracecol=NA,
          key.title = "Discriminability",     
          ...
          breaks = col.map$palette.breaks,
          srtCol=1,  
          RowSideColors = c("mediumpurple4","darkolivegreen","darkolivegreen3","mediumpurple")[cases$class[indx]])
...
d$id <- rownames(d)
d <- d[,c(8:9,1:7)]
d <- reshape(d, direction = "long", varying = list(names(d)[3:9]), v.names = "response", 
        idvar = c("type","id"), timevar = "fts", times = c(0,2,4,8,16,32,64))
rownames(d) <- NULL

# LME with random intercep
fst.lme <- lme(response~fts,
                random=~1|type,
                data=d, method="ML")
# LME with random slope
fst.lme1 <- lme(response~fts,
                random=~fts|type,
                data=d, method="ML")
# Plot of the response based on fine structure given the group
xyplot(response~fts | type, data=d, layout=c(2,2))
...
sum(diag(prop.table(count.table)))
...
# MANOVA follow up
library(tables)
m <- lm(cbind(nb0, nb2, nb4, nb8, nb16, nb32, nb64, age) ~ type, d)
m %>% manovaTable
etasq(m %>% manova)

heplot(mnv,variables =)
...
plot(cca, col = myColors, pch = rep(16,4), bty = "n", xpd = T)
